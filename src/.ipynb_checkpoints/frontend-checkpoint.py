...
API_URL = "https://diabetes-readmission-pipeline.onrender.com"


st.title("🏥 Diabetes Patient Readmission Risk Calculator")
st.markdown(
...
with col1:
    ...
    st.markdown("### 📈 Prior Utilization History")
    ...
    st.markdown("### 💊 Medications & Diagnosis Clusters")
    med_col1, med_col2, med_col3 = st.columns(3)
    with med_col1:
        insulin = st.checkbox("Prescribed Insulin", value=True, key="insulin_check")
        metformin = st.checkbox("Prescribed Metformin", value=False, key="metformin_check")
    with med_col2:
        diabetesMed = st.checkbox("Any Diabetes Medication", value=True, key="diabetes_med_check")
        diag_1_group_Diabetes = st.checkbox("Primary Diagnosis: Diabetes", value=True, key="diag1_check")
    with med_col3:
        diag_2_group_Circulatory = st.checkbox("Secondary Diagnosis: Circulatory Disease", value=False, key="diag2_check")
...
with col2:
    ...
    if st.button("Run Inference", type="primary", use_container_width=True, key="run_inference_btn"):
        try:
            with st.spinner("Evaluating models across decision baselines..."):
                response = requests.post(f"{API_URL}/predict", json=payload, timeout=15.0)
            ...
            if response.status_code == 200:
                ...
                st.metric(label="Calculated Readmission Risk", value=f"{prob * 100:.1f}%")
                
                # Dynamic alert coloring based on standard baseline
                if prob > 0.50:
                    st.error("⚠️ HIGH RISK: Immediate post-discharge support recommended.")
                elif prob > 0.45:
                    st.warning("⚠️ ELEVATED RISK: Flagged under aggressive screening thresholds.")
                else:
                    st.success("✅ LOW RISK: Standard discharge workflow acceptable.")
                ...
                st.info(f"**Aggressive Screening (0.45):** {render_flag(decisions['threshold_0_45'])}")
                st.info(f"**Standard Balanced Baseline (0.50):** {render_flag(decisions['threshold_0_50'])}")
                st.info(f"**Resource-Constrained Target (0.55):** {render_flag(decisions['threshold_0_55'])}")
                
            else:
                st.error(f"Prediction Service returned error code: {response.status_code}")
        except Exception as e:
            st.error(f"Could not reach backend API layer. Is the FastAPI service active? Error: {e}")